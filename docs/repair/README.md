...
7. Freezes exactly one worker run with the hydrated job and target repository
   in a trusted pre-execution manifest. The Codex execution job receives that
   artifact and an exact-repository target token, but no state token or
   persisted state credential.
8. Executes credited fix artifacts through `scripts/execute-fix-artifact.ts`
   when the fix gate is open, then seals the exact run tree. A separate
   credential-free job fetches the published PR head plus current
   `origin/main`, replays the deterministic validation plan, and emits a
   digest-bound receipt.
9. Publishes failed or cancelled execution as report-only. Guarded close,
   comment, tag, requeue, and explicit merge actions run in a token-only job
   only after execution, fix outcome, sealed handoff, and independent proof all
   succeed.
10. Publishes a sanitized result ledger back to `openclaw/clawsweeper-state`
...
Target validation runs through a versioned staged proof plan instead of artifact order. The plan parses only allowlisted read-only commands, rejects snapshot/formatter mutation flags, package-manager cwd/prefix/config path overrides, mutating aliases such as `npm i` or `npm rm`, deployment and pack commands, and install/publish lifecycle scripts invoked through `run`. It retains every required artifact/profile command, deduplicates exact argv, and orders repository integrity, static analysis, focused tests, the repository's canonical changed-surface gate, and broad/live/e2e proof from cheap to expensive. Repository-profile commands are classified by their parsed behavior; only the configured changed gate is canonical. Supported `${VAR:-default}` arguments are resolved directly from the sanitized child environment without invoking a shell, then the concrete argv is revalidated, digested, planned, and executed. Package-manager global options are parsed before classifying the effective command or wrapper; workspace selectors are preserved and their script existence is left to the selected workspace's package manager instead of being checked against only the root `package.json`. Narrow source/test repairs run focused tests early; migration, schema, security, workflow, dependency, and broad-surface repairs retain stronger supplied commands. Execution fails fast on a failed prerequisite, a canonical changed-surface stall, a command that changes the checkout/head/pinned base, or a command that finishes beyond the total proof budget. Exhausted command and proof budgets are terminal and cannot spawn another validation or Codex fix attempt. A later command is skipped only when `config/target-repositories.json` carries an exact toolchain subsumption contract; arbitrary tests are never inferred redundant, and structured integration, QA, live, docker, or e2e proof is never subsumed. Required changed-surface, security, review, exact-head, GitHub-check, and live-proof gates remain intact.

Repository contracts use `proof_subsumptions: [{ "command": "...", "subsumes": ["..."] }]` beside `validation_commands` and `changed_gate`. Both sides must parse through the normal validation allowlist. Contracts are ignored when malformed, and they never skip repository-integrity, canonical changed-surface, elevated-risk, live, docker, integration, or e2e proof.

Each run persists a bounded schema-v2 `validation_proof` trace in the fix execution report and merge preflight: exact validated head/base SHAs, risk signals, command ID/digest/kind/source, stage, status, duration, reason, prerequisite, and digest-only subsumption provenance. Command IDs must carry the command-digest prefix, and the plan ID is recomputed from the original DAG topology before authorization, including failed attempts whose later commands were transitively skipped. It contains no raw logs, command arguments, credentials, absolute paths, or hosts. Operator logs and prompts receive only compact stage/count summaries. Before push, the executor reruns staged proof whenever the final checkpoint or either normal or push-denied fallback history compaction changed the validated head, then rebinds merge preflight to that proof. Repair merge post-flight requires a passed trace whose head matches both the fix action and live PR head and whose base matches the live PR base; stale or internally inconsistent proof blocks merge. Every direct repair merge owner additionally requires a verifiable GitHub App installation and strict required status checks from an effective non-bypass repository ruleset or classic branch protection, so a base advance between validation and merge is rejected by the server; absent, bypassed, inherited-only, or unreadable strict rules disable automerge. Administration verification runs in a separate trusted job that validates the exact target repository before minting repository-scoped mutation and verifier tokens; the job does not set up Codex or execute target-repository code. Earlier failed attempts remain visible after a successful repair retry.
