...
Target validation runs through a versioned staged proof plan instead of artifact order. The plan parses only allowlisted commands, retains every required artifact/profile command, deduplicates exact argv, and orders repository integrity, static analysis, focused tests, the repository's canonical changed-surface gate, and broad/live/e2e proof from cheap to expensive. Narrow source/test repairs run focused tests early; migration, schema, security, workflow, dependency, and broad-surface repairs retain stronger supplied commands. Execution fails fast on a failed prerequisite. A later command is skipped only when `config/target-repositories.json` carries an exact toolchain subsumption contract; arbitrary tests are never inferred redundant, and required changed-surface, security, review, exact-head, GitHub-check, and live-proof gates remain intact.
...
Each run persists a bounded `validation_proof` trace in the fix execution report and merge preflight: schema and plan version, risk signals, command ID/digest/kind, stage, status, duration, reason, prerequisite, and digest-only subsumption provenance. It contains no raw logs, command arguments, credentials, absolute paths, or hosts. Operator logs and prompts receive only compact stage/count summaries. Post-flight accepts legacy preflights without a trace, but when a staged trace is present its latest run must be complete, topologically consistent, and passed; earlier failed attempts remain visible after a successful repair retry.
