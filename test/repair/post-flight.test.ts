...
test("merge post-flight requires exact proof and server-enforced strict base binding", () => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "    ? [{ type: 'required_status_checks', ruleset_id: 18588237, ruleset_source: 'openclaw/openclaw', ruleset_source_type: 'Repository', parameters: { strict_required_status_checks_policy: true, required_status_checks: [{ context: 'clownfish/exact-merge' }] } }]",
      "    : [];",
      "  process.stdout.write(JSON.stringify(rules));",
      "  process.exit(0);",
      "}",
      "if (args[0] === 'api' && args[1] === 'repos/openclaw/openclaw/rulesets/18588237') {",
      "  process.stdout.write(JSON.stringify({ bypass_actors: [] }));",
      "  process.exit(0);",
      "}",
