...
function executeCommand(command: LooseRecord) {
  try {
    ...
    if (
      ...
      if (applyAutomergeResultToCommand(command, merge)) return;
...
function executeAutomerge(command: LooseRecord): LooseRecord {
  ...
  const mergeClaim = claimAutomergeMergeRequest(command);
  ...
  const liveDispatchStateBlock = (): { reason: string; retryable: boolean } | null => {
    let state;
    try {
  ...
  const finalDispatchSafetyBlock = (): {
    ...
    if (!verifiedBaseBranch || finalBaseBranch !== verifiedBaseBranch) {
      return {
  ...
  if (automergeCommandResponseAmbiguous(result)) {
    ...
    if (automergeUnconfirmedFailureDisposition(result) === "waiting") {
      return {
        action: "merge",
        status: "waiting",
        reason: `merge command response was transient and its exact-head effect is not yet observable: ${failure}`,
        merge_method: "squash",
        transient_wait_ms: waitedMs,
        transient_observations: transientObservations,
      };
...
function automergeMergeClaimRequest(command: LooseRecord): ExactHeadMergeClaimRequest {
  return {
    ...
    number: Number(command.issue_number),
    headSha: String(command.expected_head_sha ?? ""),
    method: "squash",
    owner: "comment_router",
    claimant: exactHeadMergeClaimant("comment_router"),
    appId: Number(process.env.CLAWSWEEPER_AUTHENTICATED_APP_ID),
    appSlug: String(process.env.CLAWSWEEPER_AUTHENTICATED_APP_SLUG ?? ""),
  };
}

function reconcileClaimedAutomergeRequest(
  command: LooseRecord,
  ...
  try {
    ...
    if (confirmation.pendingReason) {
      return automergePendingAction(confirmation.pendingReason, waitedMs, transientObservations);
    }
  } catch (error) {
    return {
      action: "merge",
      status: "waiting",
      reason: `claimed merge request could not be reconciled from GitHub: ${compactGhError(error)}`,
      merge_method: "squash",
      transient_wait_ms: waitedMs,
      transient_observations: transientObservations,
    };
